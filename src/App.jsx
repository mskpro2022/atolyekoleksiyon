...
const VURGU_RENKLERI = {
  ...
  turkuaz:  { l:"Turkuaz",  ana:"#40c8c0", koyu:"#2ba39c" },
  turuncu:  { l:"Turuncu",  ana:"#ff9f0a", koyu:"#e0850a" },
...
function buildKonfHTML(siparis, altinKgUSD, mc, fiyatli) {
  ...
  const css = [
    ...
    ".doc-no{font-size:13px;font-weight:600;color:#0a84ff;letter-spacing:.04em}",
    ...
    ".tas-gr{color:#0a84ff;font-weight:600}",
  ...
  h += "</div>";
  if (siparis.aciklama) h += "<div style='margin:0 0 12px;padding:8px 12px;background:#f8f8f8;border-left:3px solid #0a84ff;border-radius:4px;font-size:11px;color:#444'><b>Açıklama:</b> " + siparis.aciklama + "</div>";
      ...
      if (taslar.length > 0) {
        ...
        if (gosterTas > 0) {
          tasStr += "<div style='margin-top:4px;padding:3px 7px;background:#eef4fa;border-radius:3px;font-size:8px;color:#4a5568'>"
            + "Toplam taş: <span style='color:#0a84ff;font-weight:700'>" + fN(gosterTas,4) + " gr</span>"
...
function buildSatisRaporuHTML(modeller, siparisler) {
  ...
  const css = "*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#f0f0f0;padding:16px;display:flex;justify-content:center}"
    ...
    + ".hdr{border-bottom:2px solid #0a84ff;padding-bottom:10px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:flex-end}"
    ...
    + "th{background:#faf5e8;padding:6px 8px;font-size:8px;font-weight:700;color:#8a7d64;text-transform:uppercase;border-bottom:2px solid #0a84ff;text-align:left}"
    + "th.r{text-align:right}td{padding:7px 8px;border-bottom:1px solid #f0f0f0;font-size:11px;vertical-align:middle}"
    + "td.r{text-align:right}.tot{background:#faf5e8;font-weight:800;border-top:2px solid #0a84ff}"
    ...
    + ".bar div{height:100%;background:#0a84ff;border-radius:2px}"
    + ".pb{position:fixed;bottom:16px;right:16px;background:#0a84ff;border:none;border-radius:10px;padding:10px 22px;color:#fff;font-size:13px;cursor:pointer}";
...
function buildSipListeRaporuHTML(siparisler, altinKgUSD, mc) {
  ...
  const css = "*{margin:0;padding:0;box-sizing:border-box}"
    ...
    + ".divider{border:none;border-top:2px solid #0a84ff;margin:10px 0}"
    ...
    + "th{background:#faf5e8;padding:5px 7px;font-size:7.5px;font-weight:700;color:#8a7d64;text-transform:uppercase;letter-spacing:.05em;border-bottom:2px solid #0a84ff;text-align:left}"
    + "th.r{text-align:right}"
    ...
    + ".pb{position:fixed;bottom:14px;right:14px;background:#0a84ff;border:none;border-radius:10px;padding:9px 20px;color:#fff;font-size:12px;cursor:pointer;font-family:sans-serif;font-weight:700}";
...
function buildMusteriDetayHTML(musAd, musKod, siparisler) {
  ...
  musSiparisler.forEach(s => {
    ...
    rows.forEach(({k,hc,adet,kDurum}) => {
      const renkRenk = k.renk==="Rose"?"#c0695e":k.renk==="Beyaz"?"#888":"#0968cc";
...
// ═══ AI İSİMLENDİRME ═══
// Detay noktası editörü — AYNI fotonun üzerinde sürüklenebilir/boyutlandırılabilir daire seçimi.
...
function DetayNoktaEditor({ foto, noktalar, setNoktalar, T }) {
  ...
  const ekle = (etiket) => {
    const yeni = { id: "d"+Date.now()+Math.random().toString(36).substr(2,4), etiket, tip:"kirp", cx:0.5, cy:0.5, r:0.12, foto:"" };
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:8, color:"#665d4a", marginBottom:9 }}>Daireyi sürükleyerek konumlandırın — nerede duracağı burası.</div>
      ...
      {noktalar.length > 0 && (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {noktalar.map(n => (
            <div key={n.id} onClick={()=>setAktifId(n.id)} style={{ background: aktifId===n.id?"rgba(var(--vurgu-rgb),0.1)":"rgba(255,255,255,0.04)", borderRadius:9, padding:"7px 8px", cursor:"pointer" }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
                <input value={n.etiket} onClick={e=>e.stopPropagation()} onChange={e=>setNoktalar(noktalar.map(x=>x.id===n.id?{...x,etiket:e.target.value}:x))}
                  style={{ flex:1, minWidth:0, background:"rgba(255,255,255,0.06)", border:"none", borderRadius:6, padding:"5px 8px", color:T.text, fontSize:10, outline:"none" }}/>
                <button onClick={(e)=>{ e.stopPropagation(); setNoktalar(noktalar.filter(x=>x.id!==n.id)); }} style={{ background:"none", border:"none", color:"#e85a4f", fontSize:13, cursor:"pointer", padding:"2px 5px" }}>✕</button>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }} onClick={e=>e.stopPropagation()}>
                {/* Mod seçimi: aynı fotodan kırp / ayrı foto yükle */}
                <div style={{ display:"inline-flex", background:"rgba(255,255,255,0.06)", borderRadius:6, padding:2 }}>
                  <button onClick={()=>setNoktalar(noktalar.map(x=>x.id===n.id?{...x,tip:"kirp"}:x))}
                    style={{ background: (n.tip||"kirp")==="kirp"?"var(--vurgu)":"transparent", border:"none", borderRadius:4, padding:"4px 8px", color: (n.tip||"kirp")==="kirp"?"#fff":"#998a6e", fontSize:8, fontWeight:700, cursor:"pointer" }}>🔲 Bu Fotodan Kırp</button>
                  <button onClick={()=>setNoktalar(noktalar.map(x=>x.id===n.id?{...x,tip:"foto"}:x))}
                    style={{ background: n.tip==="foto"?"var(--vurgu)":"transparent", border:"none", borderRadius:4, padding:"4px 8px", color: n.tip==="foto"?"#fff":"#998a6e", fontSize:8, fontWeight:700, cursor:"pointer" }}>📷 Ayrı Foto Yükle</button>
                </div>
                {(n.tip||"kirp")==="kirp" ? (
                  <div style={{ display:"flex", alignItems:"center", gap:3 }}>
                    <span style={{ fontSize:7, color:"#998a6e" }}>Yakınlık</span>
                    <button onClick={()=>yakinlikDegistir(n.id,+0.03)} style={{ width:20, height:20, borderRadius:5, border:"none", background:"rgba(255,255,255,0.08)", color:"#f5f5f7", fontSize:12, cursor:"pointer" }}>−</button>
                    <button onClick={()=>yakinlikDegistir(n.id,-0.03)} style={{ width:20, height:20, borderRadius:5, border:"none", background:"rgba(255,255,255,0.08)", color:"#f5f5f7", fontSize:12, cursor:"pointer" }}>+</button>
                  </div>
                ) : (
                  <label style={{ display:"inline-flex", alignItems:"center", gap:5, background:"rgba(255,255,255,0.08)", borderRadius:6, padding:"4px 9px", cursor:"pointer" }}>
                    <span style={{ fontSize:8, color: n.foto?"#6abf69":"#e85a4f", fontWeight:700 }}>{n.foto?"✓ Foto yüklendi":"Foto seç"}</span>
...
function VitrinModu({ kod, onizleme }) {
  ...
  const [seciliKlasorler, setSeciliKlasorler] = useState([]); // katalog için sıralı KOLEKSİYON id'leri [1.,2.,3.]
  const [tumGorunum, setTumGorunum] = useState(false); // "Tüm Koleksiyonları Gör" ekranı açık mı
  // Müşteri kendi siparişini verebilsin (şu an SADECE BSP) — kod hazır, aktifOnek koşuluyla açık/kapalı
  const [siparisModal, setSiparisModal] = useState(false);
  const [siparisAdetler, setSiparisAdetler] = useState({}); // { modelId: adet }
  ...
  const [vitrinDil, setVitrinDil] = useState(() => {
    try { return localStorage.getItem("vitrin_dil") || "tr"; } catch { return "tr"; }
  });
  useEffect(() => { try { localStorage.setItem("vitrin_dil", vitrinDil); } catch {} }, [vitrinDil]);
  // Ürün adı/açıklaması çeviri önbelleği — { "orijinal TR metin": "İngilizce çeviri" }, tarayıcıda kalıcı
  const [ceviriOnbellek, setCeviriOnbellek] = useState(() => {
    try { return JSON.parse(localStorage.getItem("vitrin_ceviri") || "{}"); } catch { return {}; }
  });
  ...
  const urunCevir = (orijinalMetin) => {
    ...
    return orijinalMetin; // çeviri gelene kadar orijinali göster
  };
  ...
  useEffect(() => { (async () => {
    try {
      ...
      setAktifOnek(onek);
      // Açılışta KOLEKSİYON KARTLARI gösterilir — otomatik koleksiyon açılmaz
      setAktifKol(null);
      setDurum("hazir");
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const tazele = async () => {
      ...
      try {
        ...
        setAktifKol(prev => {
          if (!prev) return prev;
          return aktifKollar.find(x => x.id === prev.id) || null; // erişim kapandıysa kart ekranına dön
        });
        // Katalog seçimindeki artık erişilemeyen koleksiyonları düşür
  ...
  if (durum === "yukleniyor") {
    return <div style={{ minHeight:"100vh", background:"#0d0b07", display:"flex", alignItems:"center", justifyContent:"center", color:GOLD2, fontFamily:"sans-serif", fontSize:14 }}>{ce("katalog_yukleniyor")}</div>;
  }
  if (durum === "gecersiz") {
    return <div style={{ minHeight:"100vh", background:"#0d0b07", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", color:"#e8dcc8", fontFamily:"sans-serif", textAlign:"center", padding:20 }}>
      ...
      <div style={{ fontSize:18, fontWeight:700, color:GOLD2, marginBottom:6 }}>{ce("gecersiz_link_baslik")}</div>
      <div style={{ fontSize:12, color:"#998a6e" }}>{ce("gecersiz_link_mesaj")}</div>
    </div>;
  }
  ...
  const kollarSirali = [...kollar].sort((a, b) => kolSonZaman(b.id) - kolSonZaman(a.id));

  // Ortak filtre (arama + gram aralığı)
  const filtreGec = (m) => {
  ...
  const tumGruplar = tumGorunum
    ? kollarSirali
        .map(k => ({ kol: k, liste: modeller.filter(m => m.ki === k.id && filtreGec(m)).sort(modelSirala) }))
        .filter(g => g.liste.length > 0)
    : [];

  // Ekranda gösterilen model listesi (PDF de bunu kullanır)
  const koldaki = tumGorunum
  ...
  const vitrinPDF = (sutun) => {
    ...
    if (liste.length === 0) { alert(ce("once_model_sec")); return; }
  ...
  const vitrinKlasorPDF = (sutun) => {
    if (seciliKlasorler.length === 0) { alert(ce("once_koleksiyon_sec")); return; }
    // Seçim sırasına göre koleksiyonları diz (1. koleksiyon önce, 2. sonra...)
    let liste = [];
    seciliKlasorler.forEach(kid => {
      const k = kollar.find(x => x.id === kid);
      if (!k) return;
      const grup = modeller.filter(m => m.ki === kid)
        .sort((a, b) => dogalSirala(b, a)) // grup içi yüksek kod üstte
        .map(m => ({ ...m, kaynakAd: k.ad }));
  ...
  // Kart ekranına dön
  const kartEkraninaDon = () => { setAktifKol(null); setTumGorunum(false); setArama(""); setGramFiltre({ min:"", max:"" }); };

  // ═══ MÜŞTERİ SİPARİŞ GÖNDERİMİ (şu an sadece BSP) ═══
  // GÜVENLİ YAZMA: tabloSiparisleriToplu SADECE upsert yapar (ekler/günceller), asla silmez.
  // tabloSiparisleriSenkron (tam senkron, listede olmayanı SİLER) BİLEREK kullanılmıyor —
  // vitrin diğer siparişlerin listesini bilmiyor, senkron kullanılsaydı hepsini silerdi.
  const siparisGonder = async () => {
    if (!vitrinMusteri || seciliModeller.length === 0) return;
    setSiparisGonderiliyor(true);
    try {
      const kalemler = seciliModeller.map(m => ({
        ...m,
        adet: Math.max(1, Number(siparisAdetler[m.id]) || 1),
        secilenAyar: aktifAyar,
      }));
      const yeniSiparis = {
        id: uid(),
        musteri: vitrinMusteri.ad || vitrinMusteri.kod,
        musKod: vitrinMusteri.kod,
        tarih: Date.now(),
        teslimTarihi: "",
        aciklama: siparisNot.trim(),
  ...
  // "Katalog Al" — kart ekranındaysa seçili koleksiyonlar, içerideyse görünen/seçilen modeller
  const katalogAl = (sutun) => {
    if (!aktifKol && !tumGorunum) {
      if (seciliKlasorler.length === 0) { alert(ce("kart_ekrani_ipucu")); return; }
      vitrinKlasorPDF(sutun);
    } else vitrinPDF(sutun);
  };
  ...
  const vKart = (m) => {
    ...
    <div key={m.id} className="vm-card">
      <div onClick={()=>{ setDetayModel(m); if(vitrinMusteri && !onizleme) vitrinAktiviteKaydet(vitrinMusteri.onek, vitrinMusteri.kod, vitrinMusteri.ad, "model", kolAdi, m.kod, m.ad); }}
        ...
        {yeni && !cokKucuk && <span style={{ position:"absolute", top: kucuk?6:10, left: kucuk?6:10, background:"var(--vurgu)", color:"#1d1d1f", fontSize: kucuk?9:12, padding: kucuk?"3px 8px":"5px 13px", borderRadius:980, fontWeight:700, letterSpacing:"0.05em", boxShadow:"0 2px 10px rgba(0,0,0,0.35)" }}>{ce("yeni")}</span>}
        ...
        {Array.isArray(m.setParcalari) && m.setParcalari.length>0 ? (() => {
          ...
          return (
            <span style={{ fontSize:gramBoyut, color:"var(--vt1)", fontWeight:600, letterSpacing:"-0.01em", whiteSpace:"nowrap" }}>
              ...
              <span style={{ fontSize: cokKucuk?7:9, color:"var(--vt2)", fontWeight:800, marginLeft:5, letterSpacing:"0.03em", border:"1px solid rgba(var(--voverlay-rgb),0.25)", borderRadius:4, padding:"1px 4px" }}>{ce("set_etiket")}</span>
            </span>
          );
        })() : (
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--vb)", color:"var(--vt1)", fontFamily:"-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Segoe UI', sans-serif", WebkitFontSmoothing:"antialiased", "--vb":vRenkler.vb, "--vt1":vRenkler.vt1, "--vt2":vRenkler.vt2, "--vt3":vRenkler.vt3, "--vcard":vRenkler.vcard, "--voverlay-rgb":vRenkler.voverlay, "--vfloatbg":vRenkler.vfloat }}>
      ...
      <div className="vm-pad" style={{ padding:"26px 28px 6px" }}>
        {(aktifKol || tumGorunum) && (
          <button onClick={kartEkraninaDon}
            ...
            <span style={{ fontSize:19, lineHeight:1, marginTop:-1 }}>‹</span> {ce("tum_koleksiyonlar")}
          </button>
        )}
      ...
      <div className="vm-pad vm-filtre" style={{ padding:"12px 28px 14px", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
        ...
        <input value={arama} onChange={e=>setArama(e.target.value)} placeholder={ce("ara")}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(var(--voverlay-rgb),0.07)", borderRadius:9, padding:"5px 11px" }}>
          <span style={{ fontSize:12, color:"var(--vt3)" }}>{ce("gr_kisa")}</span>
      ...
      {/* ═══ KART EKRANI — izin verilen koleksiyonlar ═══ */}
      {!aktifKol && !tumGorunum && (() => {
        const toplamModel = modeller.length;
        const kapakAl = (kid) => [...modeller.filter(m => m.ki === kid && m.foto)].sort((a,b)=>(b.t||0)-(a.t||0)).slice(0,4).map(m => m.foto);
        const tumKapaklar = kollarSirali.reduce((acc, k) => acc.concat(kapakAl(k.id).slice(0,1)), []).slice(0,4);
        return (
        <div>
          {/* Katalog seçim çubuğu — koleksiyon seçilince */}
                ...
                <div style={{ width:78, height:58, borderRadius:10, overflow:"hidden", background:"#f7f7f8", display:"grid", gridTemplateColumns:"1fr 1fr", gridTemplateRows:"1fr 1fr", gap:1, flexShrink:0 }}>
                  {[0,1,2,3].map(i => (
                    <div key={i} style={{ overflow:"hidden", background:"#f0f0f0" }}>
                      {tumKapaklar[i] ? <img src={tumKapaklar[i]} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}/> : null}
                    </div>
                  ))}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:17, fontWeight:600, color:"var(--vt1)", letterSpacing:"-0.01em" }}>{ce("tum_koleksiyonlari_gor")}</div>
          ...
          <div className="vm-kart-grid vm-pad" style={{ padding:"14px 28px 8px" }}>
            {kollarSirali.map(k => {
              ...
              const sira = seciliKlasorler.indexOf(k.id); // -1 = seçili değil
          ...
          {kollarSirali.length === 0 && (
            <div style={{ textAlign:"center", color:"var(--vt3)", padding:"60px 0", fontSize:14 }}>{ce("henuz_koleksiyon_yok")}</div>
          )}
        </div>
        );
      })()}
      ...
      {(aktifKol || tumGorunum) && secili.size > 0 && (
        <div style={{ margin:"0 28px 16px", background:"rgba(var(--vurgu-rgb),0.12)", borderRadius:11, padding:"11px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {aktifOnek === "bsp2_" && (
              <button onClick={()=>setSiparisModal(true)} style={{ background:"var(--vcard)", border:"none", borderRadius:8, padding:"7px 16px", color:"var(--vt1)", fontSize:12, fontWeight:700, cursor:"pointer" }}>{ce("siparis_ver_emoji")}</button>
            )}
            <button onClick={()=>vitrinPDF(3)} style={{ background:"var(--vurgu)", border:"none", borderRadius:8, padding:"7px 16px", color:"#1d1d1f", fontSize:12, fontWeight:500, cursor:"pointer" }}>{ce("secilenlerden_pdf")}</button>
      ...
      {aktifKol && (
        <div className="vm-pad" style={{ padding:"0 28px 40px" }}>
          {koldaki.length === 0
            ? <div style={{ textAlign:"center", color:"var(--vt3)", padding:"60px 0", fontSize:14 }}>{ce("model_bulunamadi")}</div>
      ...
      {tumGorunum && (
        <div className="vm-pad" style={{ padding:"0 28px 40px" }}>
          {tumGruplar.length === 0 && <div style={{ textAlign:"center", color:"var(--vt3)", padding:"60px 0", fontSize:14 }}>{ce("model_bulunamadi")}</div>}
      ...
      {(aktifKol || tumGorunum) && (
        <div style={{ position:"fixed", right:16, bottom:20, zIndex:50, display:"flex", flexDirection:"column", background:"var(--vfloatbg)", backdropFilter:"blur(12px)", WebkitBackdropFilter:"blur(12px)", borderRadius:16, boxShadow:"0 6px 24px rgba(0,0,0,0.5)", border:"1px solid rgba(var(--voverlay-rgb),0.12)", overflow:"hidden" }}>
          <button onClick={()=>setVitrinSutun(s=>Math.max(1,s-1))} disabled={vitrinSutun<=1}
            title={ce("buyut")} style={{ width:48, height:48, border:"none", background:"transparent", color: vitrinSutun<=1?"#48484a":"var(--vt1)", fontSize:26, fontWeight:300, cursor: vitrinSutun<=1?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", lineHeight:1 }}>+</button>
          ...
          <button onClick={()=>setVitrinSutun(s=>Math.min(6,s+1))} disabled={vitrinSutun>=6}
            title={ce("kucult")} style={{ width:48, height:48, border:"none", background:"transparent", color: vitrinSutun>=6?"#48484a":"var(--vt1)", fontSize:26, fontWeight:300, cursor: vitrinSutun>=6?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", lineHeight:1 }}>−</button>
        </div>
      )}
          ...
          <div onClick={e=>e.stopPropagation()} style={{ background:"var(--vcard)", borderRadius:18, maxWidth:520, width:"100%", maxHeight:"88vh", overflow:"auto" }}>
            {siparisBasarili ? (
              <div style={{ padding:"40px 28px", textAlign:"center" }}>
                ...
                <div style={{ fontSize:18, fontWeight:600, color:"var(--vt1)", marginBottom:8 }}>{ce("siparis_alindi")}</div>
                <div style={{ fontSize:14, color:"var(--vt2)", marginBottom:24 }}>{ce("en_kisa_surede")}</div>
              ...
              <div style={{ padding:"22px 24px 24px" }}>
                <div style={{ fontSize:18, fontWeight:600, color:"var(--vt1)", marginBottom:4 }}>{ce("siparis_ver")}</div>
                ...
                <button disabled={siparisGonderiliyor} onClick={siparisGonder}
                  ...
                  {siparisGonderiliyor ? ce("gonderiliyor") : ce("siparisi_gonder")}
                </button>
              </div>
            )}
          </div>
      ...
      {detayModel && (() => {
        ...
        return (
        <div onClick={()=>setDetayModel(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:20, backdropFilter:"blur(20px)" }}>
            ...
            {dIdx > 0 && typeof window !== "undefined" && window.innerWidth > 760 && (() => { const pm = koldaki[dIdx-1]; return (
              <div onClick={(e)=>{ e.stopPropagation(); detayKomsu(-1); }} title={ce("onceki")}
                style={{ width:"min(140px,15vw)", height:"min(420px,70vh)", flexShrink:0, cursor:"pointer", opacity:0.4, filter:"blur(2px)", borderRadius:16, overflow:"hidden", transition:"opacity .2s, filter .2s" }}
                onMouseEnter={e=>{ e.currentTarget.style.opacity=0.75; e.currentTarget.style.filter="blur(0.5px)"; }}
                onMouseLeave={e=>{ e.currentTarget.style.opacity=0.4; e.currentTarget.style.filter="blur(2px)"; }}>
          ...
          <div onClick={e=>e.stopPropagation()}
            ...
            <div onClick={(e)=>{ e.stopPropagation(); if (detayModel.foto) setTamFotoAc(true); }} style={{ aspectRatio:"1/1", background:"#f7f7f8", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", position:"relative", cursor: detayModel.foto ? "zoom-in" : "default" }}>
            ...
            {zumNokta && (() => {
              const ayriFoto = zumNokta.tip === "foto";
              if (ayriFoto) return (
            ...
            <div style={{ padding:"12px 20px 16px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:10 }}>
                <span style={{ fontSize:14, fontWeight:500, color:"var(--vt1)", letterSpacing:"-0.02em" }}>{urunCevir(detayModel.ad)}</span>
                <span style={{ fontSize:10, color:"var(--vt3)" }}>{detayModel.kod}</span>
              </div>
                    ...
                    return (
                      <div key={refKod} style={{ background:"rgba(var(--voverlay-rgb),0.05)", border:"1px solid rgba(var(--voverlay-rgb),0.08)", borderRadius:11, padding:"12px 14px" }}>
                        <div style={{ marginBottom:10 }}>
                          ...
                          <div style={{ fontSize:11, color:"var(--vt2)" }}>{kategoriEtiket(parca.kategori)}</div>
                        </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6 }}>
                {VITRIN_AYARLAR.map(a => {
                  ...
                  const on = a.id === aktifAyar;
                  return (
                    <div key={a.id} style={{ background: on?"rgba(var(--vurgu-rgb),0.14)":"rgba(var(--voverlay-rgb),0.05)", border: on?"1px solid rgba(var(--vurgu-rgb),0.4)":"1px solid transparent", borderRadius:9, padding:"7px 6px", textAlign:"center" }}>
                      <div style={{ fontSize:9, color:"var(--vt2)", fontWeight: on?700:500 }}>{a.id.replace("K","")} Ayar</div>
                      <div style={{ fontSize:15, fontWeight: on?700:500, color:"var(--vt1)", marginTop:2, letterSpacing:"-0.02em" }}>{gg > 0 ? gg.toFixed(2) : "—"}</div>
                      <div style={{ fontSize:8, color:"var(--vt3)" }}>gram</div>
                    </div>
                  );
                })}
              </div>
              ...
              {aktifOnek === "bsp2_" && (Number(detayModel.tasGram) > 0 || (detayModel.taslar && detayModel.taslar.length > 0)) && (
                <div style={{ marginTop:10, background:"rgba(var(--voverlay-rgb),0.05)", borderRadius:9, padding:"10px 12px" }}>
                  <div style={{ fontSize:9, color:"var(--vt2)", fontWeight:600, letterSpacing:"0.04em", textTransform:"uppercase", marginBottom:8 }}>{ce("tas_bilgisi")}</div>
                  {Number(detayModel.tasGram) > 0 && (
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom: (detayModel.taslar && detayModel.taslar.length) ? 12 : 0, paddingBottom: (detayModel.taslar && detayModel.taslar.length) ? 12 : 0, borderBottom: (detayModel.taslar && detayModel.taslar.length) ? "0.5px solid rgba(var(--voverlay-rgb),0.08)" : "none" }}>
                      <span style={{ fontSize:13, color:"var(--vt2)" }}>{ce("tas_agirligi")}</span>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                      {detayModel.taslar.map((t, i) => (
                        <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:13 }}>
                          <span style={{ color:"var(--vt1)" }}>{t.sekil || ce("tas")}{t.boyut ? " · " + t.boyut : ""}</span>
                          <span style={{ color:"var(--vt2)" }}>{t.adet || 1} {ce("adet")}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <button onClick={()=>{ const ns=new Set(secili); secili.has(detayModel.id)?ns.delete(detayModel.id):ns.add(detayModel.id); setSecili(ns); }}
                style={{ width:"100%", marginTop:12, background: secili.has(detayModel.id)?"rgba(var(--voverlay-rgb),0.1)":"var(--vurgu)", border:"none", borderRadius:11, padding:"11px", color:"#1d1d1f", fontSize:13, fontWeight:500, cursor:"pointer" }}>
                {secili.has(detayModel.id) ? ce("secimden_cikar") : ce("sec")}
              </button>
            </div>
          </div>
...
function Atolye({ onSirketDegis }) {
  ...
  const realtimeZamanlayici = useRef(null);
  useEffect(() => {
    if (!loaded) return;
    const durdur = realtimeBaslat(AKTIF_SIRKET_ONEK, (tablo, payload) => {
      // Kendi yazdığımız değişiklikse (son 12 sn içinde) yok say — kendi ekranımız zaten güncel
      if (Date.now() - sonKendiYazma.current < 12000) return;
      // DEBOUNCE — art arda gelen bildirimleri birleştir, 2 sn bekle, tek tazeleme yap
      if (realtimeZamanlayici.current) clearTimeout(realtimeZamanlayici.current);
      realtimeZamanlayici.current = setTimeout(async () => {
        // Bu sırada kendi yazma olduysa yine iptal et
        if (Date.now() - sonKendiYazma.current < 12000) return;
        try {
          if (tablo === "modeller") {
    ...
    return () => { durdur(); if (realtimeZamanlayici.current) clearTimeout(realtimeZamanlayici.current); };
  }, [loaded]);
  ...
  useEffect(() => {
    ...
    if (sayfa === "vitrin" && loaded) {
      vitrinOzetYukle(); // müşteri aktivite özetleri
    }
    if (sayfa === "analiz" && loaded) {
      vitrinAnaliz(AKTIF_SIRKET_ONEK).then(setVitrinAnalizVeri);
    }
  ...
  const saveModel = () => {
    ...
    const detayFotolariYukle = async (modelId, noktalar) => {
      ...
      for (const n of noktalar) {
        if (n.tip === "foto" && n.foto && n.foto.startsWith("data:")) {
          const url = await fotoYukleStorage(n.foto, modelId + "-d-" + n.id, AKTIF_SIRKET_ONEK);
          sonuc.push({ ...n, foto: url });
        } else {
          sonuc.push(n); // "kirp" modundaki noktalar zaten sadece sayı (cx,cy,r) — yüklemeye gerek yok
        }
      }
      return sonuc;
    };
  ...
  const openEM = m => {
    ...
    setFKategori(m.kategori||"yuzuk"); setFEtiketler(m.etiketler||[]); setFDetayNoktalari(m.detayNoktalari||[]); setEditM(m); setShowMM(true);
    setFSetModu(false); setFSetKod1(""); setFSetKod2(""); setFSetM1(null); setFSetM2(null); // her model açılışında SET modu sıfırlansın — bir önceki modelden yapışık kalmasın
  };
      ...
      <div style={{ width:"100%", padding:"12px 12px 60px" }}>
        ...
        {sayfa==="vitrin" && (
          <div style={{ animation:"fadein .3s", maxWidth:1100, margin:"0 auto" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(340px, 1fr))", gap:14, alignItems:"start" }}>
              ...
              {/* MÜŞTERİLER — kalıcı linkler + analiz */}
              <div style={{ background:T.card, border:"1px solid "+T.border, borderRadius:14, padding:"15px 16px", gridColumn:"1/-1" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:T.sub, letterSpacing:"0.05em", textTransform:"uppercase" }}>Müşterilerim ({Object.keys(musteriler).length})</div>
                  <button onClick={()=>vitrinOzetYukle()} style={{ ...GH, fontSize:9, padding:"4px 10px" }}>↻ Analizi Yenile</button>
                </div>
                <div style={{ fontSize:9, color:"#665d4a", marginBottom:12 }}>Her müşterinin kalıcı vitrin linki var. Linki gönderin, girdiklerinde neye baktıklarını buradan izleyin.</div>

                {Object.keys(musteriler).length === 0 && <div style={{ fontSize:10, color:"#665d4a", padding:"12px 0" }}>Henüz müşteri yok. Müşteriler sayfasından ekleyin.</div>}

                {Object.keys(musteriler).length > 0 && (
                        ...
                        <div key={kod} style={{ background:"rgba(0,0,0,0.15)", border:"1px solid "+T.border, borderRadius:9, padding:"10px 12px", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                          ...
                          <div style={{ minWidth:150, flex:"1 1 180px" }}>
                            ...
                            <div style={{ fontSize:9, color: oz && oz.giris>0 ? "#6abf69" : "#665d4a", marginTop:3 }}>
                              {oz === null ? "—" : oz.giris > 0
                                ? oz.giris + " kez girdi · " + oz.model + " modele baktı · son: " + (oz.son ? new Date(oz.son).toLocaleDateString("tr-TR") : "—")
                                : "henüz vitrine girmedi"}
                            </div>
                          </div>
                          {/* Butonlar */}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, flexWrap:"wrap", gap:6 }}>
              ...
              <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
                ...
                {siparisler.length > 0 && (
                  ...
                  }} style={{ background:"linear-gradient(135deg,#0a84ff,#0968cc)", border:"none", borderRadius:9, padding:"5px 11px", color:"#0a0a0a", fontSize:9, fontWeight:800, cursor:"pointer", whiteSpace:"nowrap" }}>
                    📊 Rapor PDF
                  </button>
                )}
              </div>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            ...
                            {s.kaynak === "vitrin" && <span style={{ fontSize:9, fontWeight:800, color:"#fff", background:"linear-gradient(135deg,#a78bfa,#8b6fe8)", padding:"2px 8px", borderRadius:980 }}>🛍️ Vitrinden</span>}
                  ...
                  <div key={kod} style={{ background:"rgba(var(--vurgu-rgb),0.02)", border:"1px solid rgba(var(--vurgu-rgb),0.08)", borderRadius:10, padding:"10px 14px", display:"flex", alignItems:"center", gap:12 }}>
                    ...
                    <div style={{ display:"flex", gap:5, flexShrink:0 }}>
                      ...
                      <button onClick={async()=>{
                        ...
                        setVitrinGecmis({ musteriAd:ad, kod, kayitlar, encok });
                      }} style={{ background:"rgba(91,155,213,0.1)", border:"1px solid rgba(91,155,213,0.25)", borderRadius:5, padding:"5px 10px", color:"#5b9bd5", fontSize:9, fontWeight:700, cursor:"pointer" }}>📊 Geçmiş</button>
        ...
        {sayfa==="analiz" && (
          <div style={{ animation:"fadein .3s" }}>
            ...
            {/* ── VİTRİN ANALİZİ — müşteriler neye baktı ── */}
            {vitrinAnalizVeri && vitrinAnalizVeri.toplamModel > 0 && (
              <div style={{ background:T.card, border:"1px solid "+T.border, borderRadius:14, padding:"16px 18px", marginBottom:16 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4, flexWrap:"wrap", gap:8 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:T.sub, letterSpacing:"0.05em", textTransform:"uppercase" }}>🛍 Vitrin Analizi — Müşteriler Neye Baktı</div>
                  <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                    <span style={{ fontSize:10, color:"#5b9bd5", fontWeight:700 }}>{vitrinAnalizVeri.toplamGiris} ziyaret</span>
                    <span style={{ fontSize:10, color:"#6abf69", fontWeight:700 }}>{vitrinAnalizVeri.toplamModel} model görüntüleme</span>
                    <button onClick={()=>vitrinAnaliz(AKTIF_SIRKET_ONEK).then(setVitrinAnalizVeri)} style={{ ...GH, fontSize:9, padding:"3px 9px" }}>↻</button>
                  </div>
                </div>
                <div style={{ fontSize:9, color:"#665d4a", marginBottom:14 }}>Toptancıların vitrinde en çok incelediği modeller. Talep sinyali — hangi modeller ilgi çekiyor?</div>

                {/* EN ÇOK BAKILAN MODELLER — FOTOĞRAFLI */}
                    ...
                    return (
                      <div key={vm.kod} onClick={()=>model && openEM(model)} style={{ background:"rgba(0,0,0,0.15)", borderRadius:10, overflow:"hidden", cursor: model?"pointer":"default", border:"1px solid "+(i<3?"rgba(232,131,58,0.3)":T.border), position:"relative" }}>
                        ...
                        <div style={{ padding:"7px 9px" }}>
                          <div style={{ fontSize:9, color:GOLD, fontWeight:700, marginBottom:2 }}>{vm.kod}</div>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                            <span style={{ fontSize:11, color:"#e8833a", fontWeight:800 }}>{vm.sayi}× bakıldı</span>
                            <span style={{ fontSize:9, color:T.sub }}>{vm.musteriSayisi} müşteri</span>
                          </div>
                        </div>
                      </div>
                    );
                ...
                {/* MÜŞTERİ + KOLEKSİYON ÖZETİ */}
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:12 }}>
                  <div>
                    <div style={{ fontSize:9, color:T.sub, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:7 }}>En Aktif Müşteriler</div>
                    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                      {vitrinAnalizVeri.musteriler.slice(0,6).map(mu => (
                        <div key={mu.kod} onClick={async()=>{
                          setVitrinGecmis({ musteriAd:mu.ad||mu.kod, kod:mu.kod, kayitlar:null, encok:null });
                          const [kayitlar, encok] = await Promise.all([
                            vitrinGecmisiGetir(AKTIF_SIRKET_ONEK, mu.kod, 100),
                            vitrinEnCokBakilan(AKTIF_SIRKET_ONEK, mu.kod)
                          ]);
                          setVitrinGecmis({ musteriAd:mu.ad||mu.kod, kod:mu.kod, kayitlar, encok });
                        }} style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 10px", background:"rgba(91,155,213,0.06)", borderRadius:7, cursor:"pointer" }}>
                          <span style={{ fontSize:11, color:T.text, fontWeight:700, flex:1, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{mu.ad || mu.kod}</span>
                          <span style={{ fontSize:9, color:"#5b9bd5", fontWeight:700 }}>{mu.model} model</span>
                          <span style={{ fontSize:9, color:T.sub }}>{mu.giris}× giriş</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize:9, color:T.sub, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:7 }}>En Çok Açılan Koleksiyonlar</div>
                    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                      {vitrinAnalizVeri.koleksiyonlar.slice(0,6).map(kl => {
                        const maxSay = vitrinAnalizVeri.koleksiyonlar[0]?.sayi || 1;
                        return (
                          <div key={kl.ad} style={{ padding:"6px 10px", background:"rgba(106,191,105,0.06)", borderRadius:7, position:"relative", overflow:"hidden" }}>
                            <div style={{ position:"absolute", left:0, top:0, bottom:0, width:(kl.sayi/maxSay*100)+"%", background:"rgba(106,191,105,0.1)" }}></div>
                            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", position:"relative" }}>
                              <span style={{ fontSize:11, color:T.text, fontWeight:600, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{kl.ad}</span>
                              <span style={{ fontSize:9, color:"#6abf69", fontWeight:700, marginLeft:8 }}>{kl.sayi}×</span>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  {Object.entries(VURGU_RENKLERI).map(([id, v]) => {
                    ...
                    return (
                      <button key={id} onClick={()=>setVurguRenk(id)} title={v.l} style={{
                        ...
                        <div style={{ width:34, height:34, borderRadius:"50%", background:v.ana, border: aktif?"3px solid #fff":"2px solid rgba(255,255,255,0.15)", boxShadow: aktif?"0 0 0 2px "+v.ana:"none" }}/>
                        <span style={{ fontSize:9, color: aktif?T.text:T.sub, fontWeight: aktif?700:400 }}>{v.l}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              {AKTIF_SIRKET_ONEK && (
                <button onClick={async()=>{
                  ...
                  try {
                    ...
                    if (mskAy.varsAltinKg) setAyarVarsAltinKg(mskAy.varsAltinKg);
                    if (mskAy.varsMc) setAyarVarsMc(mskAy.varsMc);
                    if (mskAy.varsIscilik) setAyarVarsIscilik(mskAy.varsIscilik);
                    if (mskAy.varsIscilikBirim) setAyarVarsIscilikBirim(mskAy.varsIscilikBirim);
                    alert("Kopyalandı ✓\nÖzel taş: " + (mskAy.ozelTaslar?.length||0) + "\nKategori: " + (mskAy.kategoriler?.length||0) + "\nNot: " + (mskAy.kayitliNotlar?.length||0));
                  } catch(e) { alert("Hata: " + e.message); }
                }} style={{ width:"100%", background:"rgba(var(--vurgu-rgb),0.12)", border:"1px solid rgba(var(--vurgu-rgb),0.35)", borderRadius:10, padding:"11px 14px", color:"var(--vurgu)", fontSize:12, fontWeight:700, cursor:"pointer", marginBottom:12 }}>
                  📥 MSK ayar/taşlarını bu şirkete kopyala
                </button>
              )}
              {ozelTaslar.length === 0 && (
                <button onClick={()=>{
      ...
      {vitrinGecmis && (
        <div onClick={()=>setVitrinGecmis(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20, backdropFilter:"blur(4px)" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:T.card, borderRadius:16, maxWidth:600, width:"100%", maxHeight:"88vh", overflow:"auto", border:"1px solid "+T.border, position:"relative" }}>
            <div style={{ padding:"20px 24px", borderBottom:"1px solid "+T.border, position:"sticky", top:0, background:T.card, zIndex:2 }}>
              <button onClick={()=>setVitrinGecmis(null)} style={{ position:"absolute", top:16, right:18, width:32, height:32, borderRadius:"50%", background:"rgba(0,0,0,0.3)", border:"none", color:T.text, fontSize:16, cursor:"pointer" }}>✕</button>
              <div style={{ fontSize:10, color:"#5b9bd5", fontWeight:800, letterSpacing:"0.05em", textTransform:"uppercase" }}>📊 Vitrin Geçmişi</div>
              <div style={{ fontSize:19, fontWeight:800, color:T.text, marginTop:3 }}>{vitrinGecmis.musteriAd}</div>
              <div style={{ fontSize:11, color:T.sub }}>{vitrinGecmis.kod}</div>
            </div>
            <div style={{ padding:"18px 24px" }}>
              {vitrinGecmis.kayitlar === null && <div style={{ textAlign:"center", color:T.sub, padding:"30px", fontSize:13 }}>Yükleniyor...</div>}
              ...
              {vitrinGecmis.kayitlar !== null && vitrinGecmis.kayitlar.length > 0 && (
                <>
                  ...
                  {vitrinGecmis.encok && vitrinGecmis.encok.length > 0 && (
                    <div style={{ marginBottom:20 }}>
                      <div style={{ fontSize:10, color:T.sub, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:8 }}>🔥 En Çok İlgilendiği Modeller</div>
                      <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                        {vitrinGecmis.encok.map((e,i) => (
                          <div key={i} style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 11px", background:"rgba(232,131,58,0.06)", borderRadius:8 }}>
                            <span style={{ fontSize:11, fontWeight:800, color:"#e8833a", minWidth:20 }}>{i+1}.</span>
                            <span style={{ fontSize:9, color:GOLD, fontWeight:700 }}>{e.kod}</span>
                            <span style={{ fontSize:12, color:T.text, flex:1, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{e.ad}</span>
                            <span style={{ fontSize:10, color:"#e8833a", fontWeight:700 }}>{e.sayi}× baktı</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Zaman çizelgesi */}
                  <div style={{ fontSize:10, color:T.sub, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:8 }}>Son Aktiviteler ({vitrinGecmis.kayitlar.length})</div>
                  <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                    {vitrinGecmis.kayitlar.map(a => {
                      const ikon = a.eylem==="giris" ? "🚪" : a.eylem==="koleksiyon" ? "📂" : "💎";
                      const metin = a.eylem==="giris" ? "Vitrine girdi" : a.eylem==="koleksiyon" ? ("« "+a.koleksiyon+" » koleksiyonunu açtı") : ((a.model_kod||"")+" "+(a.model_ad||"")+" modeline baktı");
                      return (
                        <div key={a.id} style={{ display:"flex", alignItems:"center", gap:9, padding:"6px 10px", background:"rgba(0,0,0,0.12)", borderRadius:7, fontSize:11 }}>
                          <span style={{ fontSize:13 }}>{ikon}</span>
                          <span style={{ flex:1, color:T.text }}>{metin}</span>
                          <span style={{ fontSize:9, color:T.sub, whiteSpace:"nowrap" }}>{new Date(a.zaman).toLocaleString("tr-TR",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            <div onClick={e=>e.stopPropagation()} style={{ background:T.card, borderRadius:16, maxWidth:820, width:"100%", maxHeight:"92vh", overflow:"auto", border:"1px solid "+T.border, position:"relative" }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap" }}>
                ...
                <div style={{ flex:"1 1 320px", padding:"22px 24px", color:T.text }}>
                  ...
                  {/* MÜŞTERİ GİZLEME — bu modeli hangi müşteriler görmesin */}
                  <div style={{ marginBottom:14, background:"rgba(232,90,79,0.04)", border:"1px solid rgba(232,90,79,0.15)", borderRadius:10, padding:"11px 13px" }}>
                    ...
                    {Object.keys(musteriler).length === 0 && <div style={{ fontSize:9, color:T.sub }}>Müşteri yok.</div>}
                    <div style={{ display:"flex", flexWrap:"wrap", gap:5, maxHeight:110, overflowY:"auto" }}>
                      {Object.entries(musteriler).sort((a,b)=>a[0].localeCompare(b[0],"tr")).map(([mad, mkod]) => {
                        const gizli = Array.isArray(m.gizliMus) && m.gizliMus.includes(mkod);
                        return (
                  ...
                  {/* Aksiyon butonları */}
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>{ setDetayModel(null); openEM(m); }} style={{ flex:1, background:"linear-gradient(135deg,#0a84ff,#0968cc)", border:"none", borderRadius:9, padding:"11px", color:"#1a1a1a", fontSize:13, fontWeight:800, cursor:"pointer" }}>✏️ Düzenle</button>
          ...
          <div style={{ marginBottom:10 }}>
            {!fDetayAcik && fDetayNoktalari.length === 0 && (
              <button onClick={()=>setFDetayAcik(true)} style={{ background:"rgba(255,255,255,0.05)", border:"1px dashed rgba(255,255,255,0.2)", borderRadius:10, padding:"8px 12px", color:"#998a6e", fontSize:10, fontWeight:600, cursor:"pointer", width:"100%", textAlign:"left" }}>
                🔍 Detay noktası ekle (kilit, zincir, taş vs. — opsiyonel)
              </button>
            )}
            {(fDetayAcik || fDetayNoktalari.length > 0) && (
              <div style={{ background:"rgba(var(--vurgu-rgb),0.05)", border:"1px solid rgba(var(--vurgu-rgb),0.15)", borderRadius:12, padding:"12px 13px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:T.sub, letterSpacing:"0.05em", textTransform:"uppercase" }}>🔍 Detay Noktaları</div>
                ...
                <div style={{ fontSize:9, color:"#998a6e", marginBottom:9 }}>Fotonun üzerinde daireyi sürükleyerek konumlandırın — ayrı foto yüklemenize gerek yok, vitrinde o bölge otomatik yakınlaştırılır.</div>
          ...
          <div style={{ background:"rgba(232,90,79,0.06)", border:"1px solid rgba(232,90,79,0.2)", borderRadius:8, padding:"8px 10px", marginTop:-4, marginBottom:6 }}>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:6 }}>
              ...
              <button onClick={() => {
                ...
                setFEtiketler(m.etiketler||[]); setFFoto(m.foto||""); setFDetayNoktalari(m.detayNoktalari||[]);
